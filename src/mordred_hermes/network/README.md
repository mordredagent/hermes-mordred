...
| `hooks` | `on_session_start` reads policy/config/auth, keys Tor isolation, brings up `default_path`, then gates the persisted provider. `pre_api_request` repeats the strict-Tor gate against Hermes's request-resolved provider so runtime overrides cannot bypass transport evidence. Incompatible/unverified/unknown providers and internal errors fail closed after audit. Startup refusal tears down its path; request refusal keeps Tor active so later gateway events are gated again rather than falling through to clearnet. Lenient downgrades startup flags; off skips them. `on_session_end` calls `api.stop`; `pre_tool_call` refuses strict sessions after a path drop. | `tests/test_network_hooks.py` |
| `__init__.register(ctx)` | Builds `Runtime` with `RuntimeConfig` defaults, `api.set_runtime(runtime)`, registers 4 hook callbacks wired with audit writer + default config paths. | `tests/test_network_hooks.py::TestRegister` |
