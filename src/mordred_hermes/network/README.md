...
- `paths/tor.py::circuit_status_health(handle, *, controller_factory=None)` — Tor ControlPort cookie auth + `GETINFO circuit-status` deep liveness probe. After successful authentication, an empty reply or a reply containing at least one non-terminal circuit is healthy; this includes `LAUNCHED`/`EXTENDED`/`GUARD_WAIT`/`BUILT` and syntactically valid future status keywords. A reply containing only known terminal `FAILED`/`CLOSED` circuits is unhealthy. The `[tor-control]` extra (`pip install mordred-hermes[tor-control]`) supplies `stem>=1.8.0,<2`; when available, this is the runtime default. Missing stem / missing cookie falls back to shallow `process.poll()`, while authentication, ControlPort, GETINFO, and malformed-response failures are unhealthy.
