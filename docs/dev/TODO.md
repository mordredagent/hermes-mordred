...
- [x] Restore `.github/workflows/release.yml` (PyPI Trusted Publishing, M7) — workflow restored 2026-07-06 (paths adapted to the flat repo root); the one-time manual operator setup (pending publishers + GitHub Environments, CI.md §初回 setup) was completed 2026-07-07 together with the M7 name reservation (§0.5 M7)
...
- [x] `docs/UPSTREAM.md` に Hermes upstream 戦略の決定を記録 (Phase D で書き換え) — **完了**。 実ファイルは `docs/dev/UPSTREAM.md` (114 行): repository position / zero-PR commitment / optional remote 手順 / hook signature drift detection を記録済み

### 0.3 Mordred-owned filesystem paths を予約 (PATHS.md と同期)

- [x] `~/.hermes/mordred/audit.log` を Phase 1 owner として `docs/PATHS.md` に文書化済み (✓ 完了、 Phase C 内)
- [x] `~/.hermes/mordred/policy.json` (writer = `mordred_wizard`、 reader = privacy_check/llm_guard/network) を `docs/PATHS.md` に文書化済み (✓ 完了)
- [x] `~/.hermes/mordred/credentials/` を Phase 3 owner として `docs/PATHS.md` に文書化済み (✓ 完了)
- [x] `~/.hermes/mordred/keyvault/` を Phase 4 owner として `docs/PATHS.md` に文書化済み (✓ 完了)
- [x] 各 plugin の `README.md` (0.4 で作成) に own path を cross-reference

### 0.4 Plugin scaffolding (five plugins)

- [x] `src/mordred_hermes/privacy_check/` を scaffold: `plugin.yaml`, `__init__.py` (with `register(ctx)`), `policy.py`, `skill_frontmatter.py`, `audit.py`, `install_wrapper.py`, `README.md`, `tests/test_*.py`
- [x] `src/mordred_hermes/wizard/` を scaffold: `plugin.yaml`, `__init__.py`, `cli.py`, `configure.py`, `upgrade.py`, `policy_writer.py`, `policy_explainer.py`, `README.md`, `tests/test_*.py`
- [x] `src/mordred_hermes/llm_guard/` を scaffold: `plugin.yaml` (`privacy_lock: true`), `__init__.py`, `local_adapter.py`, `transport.py`, `health.py`, `override.py`, `harness_detect.py`, `README.md`, `tests/test_*.py`
- [x] `src/mordred_hermes/network/` を scaffold: `plugin.yaml`, `__init__.py`, `paths/{tor,vpn,clearnet}.py`, `proxy_env.py`, `provider_transport_flagger.py`, `api.py`, `runtime.py`, `README.md`, `tests/test_*.py`
- [x] `src/mordred_hermes/keyvault/` を scaffold: `plugin.yaml` (macOS extra で gating), `__init__.py`, `native.py` (lazy import), `api.py`, `crypto.py`, `wrap.py`, `backup.py`, `recovery.py`, `digest.py`, `seed_display.py`, `network_fallback.py`, `log_encryption.py`, `README.md`, `tests/test_*.py`
...
- [x] `pyproject.toml` 作成 (2026-05-09、 下記 deviations あり):
  - `name = "mordred-hermes"`
  - `dynamic = ["version"]` + `[tool.hatch.version] path = "docs/VERSION"` (M6: VERSION ファイルを single source of truth に)
...
- [x] **M7: PyPI name reservation** — v1 docs を public にする前に TestPyPI / PyPI で `mordred-hermes` 名を stub upload (空の `0.0.0.dev0`) で押さえる。 squat による supply-chain 攻撃の予防。 **2026-05-18: 予約 tooling 実装済み** — stub package (`packaging/name-reservation/`、 `0.0.0.dev0`、 `uv build` + `twine check` PASS 確認) + `.github/workflows/release.yml` (PyPI Trusted Publishing OIDC、 `mode=reserve`/`release` × `target=testpypi`/`pypi`、 `workflow_dispatch` 限定) + version 不変条件 test (`tests/test_packaging_versions.py`、 `0.0.0.dev0 < 0.1.0a0` を pin)。 **残**: operator が手動で PyPI/TestPyPI に pending publisher 登録 → `release.yml` を `mode=reserve` で TestPyPI→PyPI 実行 (runbook = `CI.md` §`release.yml` 詳細)。 upload 完了後に本 checkbox を `[x]` 化。 **2026-07-07 完了**: pending publisher 登録 (operator) + GitHub Environments `testpypi`/`pypi` 作成 + `mode=reserve` dispatch を TestPyPI→PyPI の順で実行、 両 index で `mordred-hermes 0.0.0.dev0` を live 確認 (詳細と required-reviewers deviation は `CI.md` §初回 setup 冒頭の完了注記)
...
- [x] `.github/workflows/ci.yml` を作成 (2026-05-09、 PR #8): pytest + pytest-cov、 ruff check / format --check、 mypy --strict、 paths-filter + concurrency cancel-in-progress、 matrix = (ubuntu-24.04 / macos-latest) × (Python 3.10 / 3.11 / 3.12)、 macOS は `[macos]` extra も install。 詳細は `docs/dev/CI.md` §`ci.yml` 詳細
...
- [x] verify 結果を `docs/SPEC.md` § "Plugin-Only Architecture" の "他に core 改修が必要になりそうな項目" にフィードバック → 2026-05-10 完了 (SPEC.md L100-107 を更新、 [`HOOK_PAYLOADS.md`](./HOOK_PAYLOADS.md) を canonical reference として cross-link)
...
- [x] DECIDE: audit log `reason` enum の最終固定。SPEC.md §Audit log policy に列挙されているコード (`policy.strict.clearnet` 等 9 個) を Phase 1 step 0 で freeze。 **2026-05-07 追記 (M2)**: `policy.strict.local_stream_interrupted` (Phase 2 mid-stream local-endpoint death) を 10 個目として追加、 同時 freeze → **2026-05-10 完了**: 12-code freeze (9 + M2 + Phase 2 forward-compat の `policy.strict.session_refused` / `policy.strict.provider_override_at_session_start` 2 個 — SPEC/PLAN で既に referenced されていたため同時に enum 化)。 `src/mordred_hermes/privacy_check/_audit_reasons.py:ReasonCode` (typed `Literal`) と `docs/dev/POLICY.md §Audit log reason enum (frozen)` が canonical
...
> **2026-05-10 Phase 1.1 PR 完了**: 全 sub-task を `src/mordred_hermes/privacy_check/` に landing。 105 テスト green、 ruff/mypy strict clean。 詳細は `docs/dev/POLICY.md` を参照。
...
- [x] `src/mordred_hermes/privacy_check/README.md` で `metadata.mordred.*` フィールド (network_requirements / requires_keyvault / outbound_endpoints) を文書化 (2026-05-10 Phase 1.1 PR、 `POLICY.md §SKILL.md \`metadata.mordred.*\` extension` も参照)
...
- [x] `docs/POLICY.md` を新規作成 (policy schema reference、 全 enum 値、 例) — 2026-05-10 Phase 1.1 PR、 12-code freeze (Phase 1 9 codes + Phase 2 forward-compat 3 codes) を `_audit_reasons.py:ReasonCode` の typed `Literal` でも宣言
- [x] `docs/VERSION` を `0.1.0a0` (PEP 440 alpha 0) で初期化。 人間可読 spec label `v0.1.0-mvp.0` は ROADMAP/SPEC/release notes 用 branding として別管理 (Codex review 2026-05-09 で確定、 PLAN §0.5 PEP 440 準拠 caveat 参照)
...
- [x] `pytest -q` 全 green、 `ruff check` / `mypy --strict src` green (2026-05-13: 282 tests passed)
...
- [ ] **`proxy_env.evaluate_library_compatibility` の本番配線判断** (2026-07-08 検出、 2026-07-13 に姉妹2件は解決): `provider_transport_flagger.evaluate` と `paths/tor.py::circuit_status_health` は **2026-07-13 に配線完了** (下記 improvement-sweep backlog 参照) — evaluate は `hooks.on_session_start` の transport-compat gate から呼ばれ strict+abort で `MordredPathBringupFailed`、 circuit_status_health は `Runtime` の `tor_health` デフォルトに昇格 (graceful self-degrade)。 残るは `proxy_env.evaluate_library_compatibility` (caller ゼロ): これは skill が宣言する HTTP ライブラリ一覧を入力に取るが、 その宣言を runtime で取得する経路が無いため配線先が未定。 v2 で `origin_skill` payload (v2-H2) が入れば skill metadata 経由で解決可 — それまで deferred するか、 別の入力源を決めるか要判断

<!-- 2026-07-13 improvement-sweep code-review deferred backlog。 6 件すべて 2026-07-13 に実装完了 (PR #42)。 詳細は `.claude/reviews/local-review-improvement-sweep.md` §Deferred。 -->
- [ ] **(follow-up) deep tor liveness の idle 誤 drop 検討** (2026-07-13 PR #42 の adversarial review MEDIUM): `Runtime` のデフォルト `tor_health` を `circuit_status_health` に昇格したことで、 `[tor-control]` extra を入れた operator の long-idle セッション (~1h 無通信で Tor が preemptive circuit を畳み `GETINFO circuit-status` が BUILT 0 本) が、 liveness worker の連続 2 回閾値で `_dropped=True` → `pre_tool_call` が `MordredPathDropped` で拒否、 という健全セッションの誤 drop を起こしうる。 stem 未導入では従来どおり shallow に縮退するので既定インストールは無影響、 かつ deep liveness を明示的に opt-in した operator の trade-off ではあるが、 「running だが今この瞬間 BUILT 回線ゼロ」を drop 扱いすべきかは要検討 (例: BUILT ゼロ + プロセス生存を「on-demand で張れる=healthy」と見なし、 auth 失敗/ControlPort が明確に壊れている時だけ unhealthy にする)
- [x] **fail-open** `paths/vpn.py::health` の Mullvad ヘルスプローブがスコープ無しの `wg show` を使い、 全 WireGuard インターフェースのうち最も新しいハンドシェイクを採用していた (`parse_handshake_age` が min を返す) — Mullvad 以外の別トンネルが 1 本でも新しければ Mullvad 自身が死んでいても healthy と誤判定し strict kill-switch の drop 検知をすり抜ける問題。 **2026-07-13 修正**: daemon 自身の `mullvad status` Connected 信号 (`wait_connected` と同じ、 sibling wg interface に騙されない) に変更。 `parse_handshake_age` は自 interface にスコープする generic-WireGuard provider が引き続き使用
- [x] **fail-open** `keyvault/extension_sign.py::_audit_sink` が監査エントリを `_log.debug` に捨て恒久ログに書かず、 拡張機能経由の署名 (`personal_sign` / `eth_signTypedData_v4` / `eth_sendTransaction`) と Secure-Enclave 認可 (`unwrap_authorized` / `unwrap_denied`) が証跡を残さなかった問題。 **2026-07-13 修正**: network/llm_guard/privacy_check と同じ暗号化対応 audit.log writer (`build_audit_writer`、 lru_cache memoize) に `safe_audit_append` で best-effort 追記 (署名は監査失敗で落とさない)
- [x] **安全なリファクタ** `keyvault/_env_write_guard.py::reseal_stray_env_if_present` が `from ..wizard import env_decrypt_cli` で wizard を runtime import し ~150 箇所の `wizard → keyvault` 依存方向を反転していた問題。 **2026-07-13 修正**: 非対話 reseal コアを新 `keyvault/_env_reseal.py::reseal_env` に降ろし、 `_env_write_guard` は直接それを呼び、 `wizard/env_decrypt_cli.reseal` は薄い wrapper に (wizard → keyvault の正方向)。 テストで wizard 非 import を静的+動的に検証
- [x] **安全なリファクタ** 監査ログの rotation / timestamp / retention が `privacy_check/audit.py` / `keyvault/log_encryption.py` / `wizard/openclaw_migration.py` に逐語重複していた問題。 **2026-07-13 修正**: pure-stdlib root module `mordred_hermes/_log_rotation.py` (`utcnow_iso` / `today_utc_date` / `next_rotation_target` / `sweep_retention`) に抽出、 3 モジュールが共有。 `log_encryption._rotate` の inline collision loop は `next_rotation_target` とバイト等価を検証してから置換、 monkeypatch seam も維持
- [x] ~~(v2) Hermes upstream の hook **payload field shape** drift を CI で監視~~ → **2026-06-12 実装済**: `tools/check_hook_payload_drift.py` (pure-`ast` で upstream の `invoke_hook("<name>", key=value, ...)` 全 dispatch site を走査、 import/install 不要) + 消費フィールド契約 `tools/hook_payload_contract.json` + `upstream-check.yml` の `Check hook payload field drift` step (drift 時は issue 本文に per-site 欠落フィールドを併記)。 `tests/test_hook_payload_drift.py` が contract キー ⇔ `register_hook` 呼出の完全一致を強制し、 canary が vendored fork へ同じ照合を毎 CI 実行
...
- [ ] (post-GA) `docs/` を topic 別 subdirectory (`strategy/` `spec/` `ops/` `hermes/` `dev/`) に再構成。 GA 前に追加する新規 doc は `HERMES_*.md` / `DEV_*.md` prefix で flat に配置し、 移行 PR で `git mv` + cross-reference 一括書き換え。 詳細は `ROADMAP.md` v2-X3
