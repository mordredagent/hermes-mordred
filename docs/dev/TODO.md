...
- [ ] **`provider_transport_flagger.evaluate` の本番配線判断** (2026-07-08 レビューで検出): severity マトリクス (`strict` tor + `respects_socks5h=False` → abort 等、 module docstring L17-27) は実装・テスト済みだが production から到達不能 — src 側の import は `wizard/configure.py` の `KNOWN_PROVIDERS` 名前リスト参照のみで、 `hooks.on_session_start` は evaluate を呼ばない。 同様に `paths/tor.py::circuit_status_health` (M9 deep liveness、 POLICY.md L239 は「runtime の `tor_health` injection で opt-in」と記載) も `Runtime(...)` 構築時に `tor_health=` を渡す production 経路がなく、 `RuntimeConfig` に対応フィールドもない。 配線する (providers は `llm_guard._resolve_active_provider` パターンで解決可、 strict-mode の新 refusal path になるため要挙動判断) か、 明示的に v2+ へ deferred と決めて本エントリを [x] 化するか。 `proxy_env.evaluate_library_compatibility` (caller ゼロ) も同判断に含める

<!-- 2026-07-13 improvement-sweep code-review deferred backlog。 挙動を変える設計判断が必要、 または安全なリファクタで、 スイープ本体とは分離。 詳細は `.claude/reviews/local-review-improvement-sweep.md` §Deferred。 上の evaluate/circuit_status_health エントリと合わせて 6 件。 -->
- [ ] **fail-open (要挙動判断)** `paths/vpn.py::health` の Mullvad ヘルスプローブがスコープ無しの `wg show` を使い、 ホスト上の**全** WireGuard インターフェースのうち最も新しいハンドシェイクを採用する (`parse_handshake_age` が min を返す)。 Mullvad 以外の別トンネルが 1 本でも新しければ、 Mullvad 自身のトンネルが死んでいても health() が healthy を返し、 strict kill-switch の drop 検知をすり抜ける。 修正案: `vpn_providers/wireguard.py::health` と同様に Mullvad の interface 名でスコープする (`mullvad status -v` 等で解決)。 挙動を変える (strict の新 drop 経路) ため要判断
- [ ] **fail-open (要挙動判断)** `keyvault/extension_sign.py::_audit_sink` が監査エントリを `_log.debug` に捨てるだけで、 恒久的な監査ログ (`EncryptedWriter`) に書かない。 拡張機能経由の `personal_sign` / `eth_signTypedData_v4` / `eth_sendTransaction` と、 それに伴う Secure-Enclave 認可 (`keyvault.unwrap_authorized` / `unwrap_denied`) が改竄検知可能な証跡を残さない。 修正案: `api` が使う監査 writer factory を `_audit_sink` に結線するか、 公開エントリポイントに sink パラメータを通す
- [ ] **安全なリファクタ** `keyvault/_env_write_guard.py::reseal_stray_env_if_present` が `from ..wizard import env_decrypt_cli` で `wizard` を runtime import しており、 他 ~150 箇所の `wizard → keyvault` 依存方向を反転している。 `keyvault/__init__.py` の `on_session_end` フックが `mordred_wizard` の importability に推移的に依存 (function-local + fail-open で degrade はする)。 修正案: `env_decrypt_cli.reseal` の非対話コアを `keyvault` (または共通 root module) に降ろし、 `wizard` 側はそれを呼ぶ形にして一方向依存に戻す
- [ ] **安全なリファクタ** 監査ログの rotation / timestamp / retention ロジックが `privacy_check/audit.py::NDJSONWriter` と `keyvault/log_encryption.py::EncryptedWriter` にほぼ逐語重複 (`_utcnow_iso` / `_today_utc_date` / `_next_rotation_target` / `_sweep_retention`、 `wizard/openclaw_migration.py` にも timestamp helper の 3 個目のコピー)。 修正案: pure-stdlib な root module (`mordred_hermes/_log_rotation.py`、 `_yaml_io.py`/`_policy_io.py` の前例に倣う) に抽出し、 `keyvault ⇄ privacy_check` の相互 import を避ける
