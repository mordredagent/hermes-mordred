...
- Hermes core (`agent/`, `hermes_cli/`, `gateway/`, etc.) never references a Mordred-owned path at all (zero-PR commitment, `MIGRATION.md` §5). Even if hard-enforcement becomes necessary in v2 and a vendored fork extra (`hermes-mordred[hard-lock]`, `vendor/hermes/<version>/`) is introduced, the patch scope is kept to localized changes in existing Hermes modules, and Mordred-specific IDs, defaults, and recovery policy are kept on the plugin side rather than placed into core (including the vendored module)
