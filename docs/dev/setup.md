...
- Python 3.11 以上 (`requires-python = ">=3.11"`、 `pyproject.toml` L10 + Hermes upstream root `pyproject.toml` も同 pin)。 CI matrix は 3.11 / 3.12 を両方 cover (`CI.md` §`ci.yml` 詳細)
- git 2.30+
...
`pyproject.toml` の `[project.optional-dependencies]` は 5 つの extra を定義する。 開発時にどれを install するかは目的次第:
...
| Lint | `ruff check src tests` | `PLAN.md` §0.6 |
| Format check | `ruff format --check src tests` | CI で blocking |
| 型検査 | `mypy --strict src` | `src/mordred_hermes/` のみ対象 |
...
- 新規 doc を追加する場合は GA まで `docs/` 直下に `HERMES_*.md` / `DEV_*.md` prefix で配置 (post-GA で `hermes/` `dev/` subdirectory に `git mv`、 `ROADMAP.md` v2-X3)
