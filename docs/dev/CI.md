...
1. **`test`** — the unit-test job across the matrix (OS × Python). ruff + mypy + pytest
2. **`fresh-venv-resolution`** — the job verifying the H1 fail-fast contract. Installs `mordred-hermes` without a root install of `hermes-agent` and asserts that dependency resolution **fails** (guaranteeing install fails fast when `hermes-agent` isn't published)
...
- **paths filter (differs by trigger type)**:
  ...
  - `push` (`main`) trigger: only `mordred-hermes/**`, `pyproject.toml`, `hermes_cli/**`, `.github/workflows/ci.yml`. **CI.md is not included in the push trigger** (docs-only changes are checked in the PR and not re-run on push to `main`)
- **concurrency**: the `ci-${{ github.ref }}` group + `cancel-in-progress: true` automatically cancels older runs on the same PR
- **`test` job — install order**: `pip install -e .` (installs Hermes upstream from root) → `pip install -e './mordred-hermes[dev]'` → `pip install -e './mordred-hermes[keyvault]'`. Since Hermes upstream (`hermes-agent`) isn't published on PyPI at this point, the root install is required (planned to be removed once `hermes-agent` itself is published on PyPI). Note that M7's `release.yml` publishes `mordred-hermes`, not `hermes-agent` (Mordred's milestone tag `v0.1.0-mvp.0` and `hermes-agent`'s version are separate things — Mordred's `hermes-agent` dependency pin is `hermes-agent>=0.11.0`; see `pyproject.toml`)
- **`keyvault` extra (all platforms)**: installs the cross-platform crypto stack (`cryptography` / `argon2-cffi` / `blake3`) on every runner, so that `mypy --strict src` / pytest can resolve the keyvault modules on Linux too (these packages themselves are cross-platform; the macOS-only restriction on keyvault *functionality* is gated by the pyobjc bridge below)
- **macOS only**: additionally runs `pip install -e './mordred-hermes[macos]'` (`macos` extra = `keyvault` extra + the pyobjc bridge for Phase 4 keyvault: `pyobjc-framework-Security` / `-SystemConfiguration` / `-Quartz`)
- **`test` job — steps**: ruff lint → ruff format check → mypy strict → pytest (coverage XML)
- **Matrix**: macOS Apple Silicon (confirms Phase 4 keyvault behavior), Ubuntu (confirms Phase 1-3 multi-platform behavior) × Python 3.11 / 3.12 (aligned with Hermes upstream's `requires-python = ">=3.11"`; mordred-hermes pins the same)
- **`fresh-venv-resolution` job**: `needs: test`. Deliberately skips `pip install -e .` and verifies that dependency resolution for `pip install -e './mordred-hermes'` fails due to the absence of `hermes-agent` (a mere non-zero exit isn't sufficient, so it greps `install.log` to confirm a `hermes-agent` dependency-resolution error actually appears)
...
- **Live tests** (`MORDRED_LIVE_LLM_TEST=1` / `MORDRED_KEYVAULT_LIVE=1` / `MORDRED_LIVE_TOR_TEST=1`) do not run by default. A separate, explicitly-triggered `workflow_dispatch` is provided
