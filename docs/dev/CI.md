...
2. **`hermes-floor`** — pins `hermes-agent==0.11.0` (the floor declared by `pyproject.toml`'s `hermes-agent>=0.11.0`), installs `mordred-hermes` on top of it, asserts the resolver did not silently upgrade it off the pin, then runs the unit suite against that exact combination. Every other job resolves the latest PyPI `hermes-agent`, so without this job the declared floor is never actually exercised
...
- **`hermes-floor` job**: standalone (no `needs:`), Python 3.12 on ubuntu-24.04. Pins `hermes-agent==0.11.0`, installs `mordred-hermes[dev,keyvault,extension]` on top of it, then a guard step reads back the installed `hermes-agent` version via `importlib.metadata` and fails the job if it isn't still `0.11.0` (which would mean the floor itself went untested). Runs the default unit suite (`pytest`; `pyproject.toml`'s `addopts` already excludes `-m integration`)
