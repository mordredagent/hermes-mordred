...
class Platform(str, Enum):      # mirrors gateway.platforms.base.Platform
    SLACK = "slack"
...
class FakeSlackAdapter:
    ...
    async def send(self, chat_id, content, reply_to=None, metadata=None):
        ...
        from types import SimpleNamespace; return SimpleNamespace(success=True)

async def main():
    from mordred_hermes.extension import crypto, pairing, e2e, outbound, gateway_plugin as gp
    ...
    ck = secrets.token_bytes(32); pairing.save_channel_key("slack:T:C0BG9QTCNKE", ck)
    tok = crypto.encrypt_message_v2(ck, "secret plan", crypto.key_id(ck))

    # 1) inbound: hook decrypts AND marks the thread (platform enum!)
    r = gp.pre_gateway_dispatch(event=Ev(text=tok, source=Src()))
