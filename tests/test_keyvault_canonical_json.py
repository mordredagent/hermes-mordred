...
invalidate artifacts already on disk, so the exact output of both named byte
forms is pinned here against a nested object containing non-ASCII text,
mixed-case keys, and non-string scalars.
"""
...
from mordred_hermes.keyvault._canonical_json import (
    canonical_json_bytes,
    canonical_json_bytes_unescaped,
    canonical_json_text,
)
...
def test_bytes_escapes_non_ascii() -> None:
    assert canonical_json_bytes(_SAMPLE) == _EXPECTED_ESCAPED.encode("utf-8")


def test_unescaped_bytes_emit_utf8() -> None:
    assert canonical_json_bytes_unescaped(_SAMPLE) == _EXPECTED_RAW.encode("utf-8")


def test_text_matches_the_byte_form() -> None:
    assert canonical_json_text(_SAMPLE) == _EXPECTED_ESCAPED
    assert canonical_json_text(_SAMPLE).encode("utf-8") == canonical_json_bytes(_SAMPLE)


def test_matches_the_historical_json_dumps_calls() -> None:
    """The exact ``json.dumps`` spellings the call sites used before the extraction."""
    assert canonical_json_bytes(_SAMPLE) == json.dumps(_SAMPLE, sort_keys=True, separators=(",", ":")).encode("utf-8")
    assert canonical_json_bytes_unescaped(_SAMPLE) == json.dumps(
...
def test_key_insertion_order_does_not_change_the_bytes() -> None:
    ...
    assert canonical_json_bytes_unescaped(reversed_order) == canonical_json_bytes_unescaped(_SAMPLE)
