...
checkout first on `PYTHONPATH`. It is the tripwire for an upstream refactor —
if the module disappears, ``MemoryStore`` grows a new read chokepoint, or a
parameter is renamed, this fails in CI rather than in an operator's memory
directory.
...
def test_journey_guard_wraps_the_real_learning_mutations(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    mutations = importlib.import_module("agent.learning_mutations")
    monkeypatch.setenv("HERMES_HOME", str(tmp_path))
