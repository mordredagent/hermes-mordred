...
Produces a streaming ``chat_handler`` for ``gateway.extension_api`` that runs
...
def make_gateway_chat_handler(runner: Any) -> Callable[[str, dict], AsyncIterator[str]]:
    ...
    async def handler(content: str, context: dict[str, Any]) -> AsyncIterator[str]:
        ...
        async with lock:
            ...
            except Exception as exc:  # noqa: BLE001 — surface a real error to the UI
            ...
            def run() -> Any:
                return agent.run_conversation(
                    message, conversation_history=history, stream_callback=stream_cb
                )
