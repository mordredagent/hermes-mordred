...
各 Mordred plugin は `mordred-hermes/src/mordred_hermes/<name>/` (pip 配布レイアウト) で以下を持つ:
...
- ルート `pyproject.toml` 例:
  ...
  [project]
  name = "mordred-hermes"
  dynamic = ["version"]                        # mordred-docs/VERSION から読む (single source of truth、 M6)
  ...
  path = "mordred-docs/VERSION"          # M6: VERSION ファイルが single source of truth、 pyproject にハードコードしない
  pattern = "^v?(?P<version>.+?)\\s*$"   # 先頭 `v` prefix を許容、 trailing whitespace を chomp。 抽出値は PEP 440 準拠であること (Hatch が validate)
  ```

  > **PEP 440 準拠 (Codex review 2026-05-09)**: `mordred-docs/VERSION` の中身は **PEP 440 準拠の version string** (例: `0.1.0a0` = alpha 0、 `0.1.0` = release、 `0.1.0.dev0` = dev release) であること。 v1 baseline は `0.1.0a0` (Hatch が `0.1.0-mvp.0` を reject するため)。 人間可読の spec label `v0.1.0-mvp.0` は ROADMAP / SPEC / release notes でのみ branding として使用、 packaging 上は `0.1.0a0` を first PyPI upload version とする。

- パッケージレイアウトは `mordred-hermes/src/mordred_hermes/{network,privacy_check,llm_guard,keyvault,wizard}/`
...
- `.github/workflows/ci.yml`:
  ...
  - `ruff check mordred-hermes/src mordred-hermes/tests`
  - `ruff format --check mordred-hermes/src mordred-hermes/tests`
...
- pytest が空でも green、 ruff/mypy も green (CI で強制、 PR #8 で landing。 詳細は `mordred-docs/dev/CI.md` §`ci.yml` 詳細)
...
- `mordred-hermes/src/mordred_hermes/privacy_check/plugin.yaml`
  ```yaml
  ...
  config_schema:
    ...
    properties:
      ...
      audit_log_path:
        type: string
  ```
- `mordred-hermes/src/mordred_hermes/privacy_check/__init__.py` — `register(ctx)` 関数
- `mordred-hermes/src/mordred_hermes/privacy_check/policy.py` — pure policy evaluator (no I/O)
- `mordred-hermes/src/mordred_hermes/privacy_check/skill_frontmatter.py` — SKILL.md frontmatter parser、 `metadata.mordred.*` 抽出
- `mordred-hermes/src/mordred_hermes/privacy_check/audit.py` — single-writer append-only audit logger with rotation
  - Phase 1–3: plaintext NDJSON, file mode `0600`
  - Writer interface: `class Writer(Protocol)`、 Phase 4 で `EncryptedWriter` に factory swap
- `mordred-hermes/src/mordred_hermes/privacy_check/install_wrapper.py` — `hermes mordred install <skill>` 実装 (wizard から呼ばれる)
...
- 文書化: `mordred-hermes/src/mordred_hermes/privacy_check/README.md` で:
...
- `mordred-hermes/src/mordred_hermes/wizard/plugin.yaml` — config under `plugins.mordred_wizard`
- `mordred-hermes/src/mordred_hermes/wizard/__init__.py` — `register(ctx)` で `ctx.register_cli_command("mordred", help="Mordred privacy layer", setup_fn=_setup_subparser, ...)`
- `mordred-hermes/src/mordred_hermes/wizard/cli.py` — `_setup_subparser(subparser)` で argparse subparser ツリー構築 (configure / upgrade / install / network / policy / audit / keyvault)
- `mordred-hermes/src/mordred_hermes/wizard/configure.py` — `hermes setup` を `subprocess.run` で child spawn、 後に Mordred-specific questions を `prompt_toolkit` (Hermes 既存依存) で
- `mordred-hermes/src/mordred_hermes/wizard/upgrade.py` — Story 1 / 1.5 single-command migration
...
- `mordred-hermes/src/mordred_hermes/wizard/policy_writer.py` — `~/.hermes/config.yaml` の `plugins.mordred_*` セクションを書き出し (`ruamel.yaml`)
- `mordred-hermes/src/mordred_hermes/wizard/policy_explainer.py` — `policy explain` / `policy dry-run` 実装
...
- E2E: `pytest mordred-hermes/tests/` 全体
...
**Files** (landed in `mordred-hermes/src/mordred_hermes/llm_guard/`):
...
- `mordred-hermes/src/mordred_hermes/network/plugin.yaml` — config under `plugins.mordred_network`、 `privacy_lock: true`
- `mordred-hermes/src/mordred_hermes/network/__init__.py` — `register(ctx)` で `on_session_start`/`on_session_end`/`pre_tool_call` 登録
- `mordred-hermes/src/mordred_hermes/network/paths/tor.py` — Tor daemon manager (v1 default = official `tor` binary、 `arti` は v2 で再評価):
...
- `mordred-hermes/src/mordred_hermes/network/paths/vpn.py` — Mullvad 公式 client wrapper (`subprocess`):
...
- `mordred-hermes/src/mordred_hermes/network/paths/clearnet.py` — no-op
- `mordred-hermes/src/mordred_hermes/network/proxy_env.py` — emits `HTTPS_PROXY` / `HTTP_PROXY` / `ALL_PROXY` / `NO_PROXY` for active path (NO_PROXY default + URL scheme は M8 §DNS / IPv6 / non-HTTP transport coverage 参照)
- `mordred-hermes/src/mordred_hermes/network/provider_transport_flagger.py` — `on_session_start` で Hermes provider adapter を列挙し proxy env vars を ignore する known provider に warning
...
- `mordred-hermes/src/mordred_hermes/network/api.py` — 内部 Python API:
...
- `mordred-hermes/src/mordred_hermes/network/runtime.py` — lazy-loaded subprocess management
...
- `mordred-hermes/src/mordred_hermes/keyvault/plugin.yaml` — `privacy_lock: true`、 macOS extra に依存
- `mordred-hermes/src/mordred_hermes/keyvault/__init__.py` — `register(ctx)` で CLI 登録、 内部 API 公開
- `mordred-hermes/src/mordred_hermes/keyvault/native.py` — `Security.framework` ラッパー (pyobjc-framework-Security 経由)、 lazy import (`_lazy_import` pattern で macOS 以外で import 時 ImportError 防止)
- `mordred-hermes/src/mordred_hermes/keyvault/api.py` — 公開 Python API:
...
- `mordred-hermes/src/mordred_hermes/keyvault/crypto.py` — AES-GCM encrypt/decrypt helpers (cryptography ライブラリ)
- `mordred-hermes/src/mordred_hermes/keyvault/wrap.py` — Secure Enclave-backed wrapping-key integration
- `mordred-hermes/src/mordred_hermes/keyvault/backup.py` — encrypted secret backup logic; Argon2id (`argon2-cffi` ライブラリ) `m=46 MiB, t=1, p=1`、 16-byte salt + verification digest を blob に embed
- `mordred-hermes/src/mordred_hermes/keyvault/recovery.py` — cross-machine recovery; `import_backup` で digest 再計算 + mismatch reject
- `mordred-hermes/src/mordred_hermes/keyvault/digest.py` — `digest = hash(hash(SeedPhrase), hash(Passphrase) ⊕ top4(PoW))` 計算 (BLAKE3 ベース)
- `mordred-hermes/src/mordred_hermes/keyvault/seed_display.py` — Seed display flow: blackout assert → 60-sec timer → display → auto-clear
- `mordred-hermes/src/mordred_hermes/keyvault/network_fallback.py` — `mordred_network.api.blackout_assert` 不在時に OS API (`SCNetworkReachability` / `nw_path_monitor` via pyobjc) を直接呼ぶ wrapper
- `mordred-hermes/src/mordred_hermes/keyvault/log_encryption.py` — Phase 1 audit `Writer` interface に slot-in する AES-GCM encryption layer。 audit-log DEK は keyvault wrapped、 メモリのみ保持
...
- `mordred-docs/UPSTREAM.md` — Hermes upstream tracking 戦略 (Phase 0)
- `mordred-docs/dev/POLICY.md` — policy schema reference (Phase 1.1 / 2026-05-10 で landed; canonical な audit log reason enum + `metadata.mordred.*` spec deviation + `plugins.mordred_privacy_check` config schema + Phase 3 `disable_ipv6` 拡張を網羅)
...
- Python >= 3.10、 strict typing (`mypy --strict mordred-hermes/src`)
...
- Mordred-as-distribution version は `mordred-docs/VERSION` で管理
