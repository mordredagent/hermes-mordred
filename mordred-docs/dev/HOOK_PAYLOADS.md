...
`mordred-docs/dev/TODO.md` §0.8 (Phase 0 acceptance gate の最後の未消化項目) は **「Hermes hook payload を実コードで verify」**。 Phase 1.1 / 2.1 / 3.1 の plugin 実装は payload 形状と return 形式に依存するため、 ここで 1 箇所に findings を集約する。
