...
- **Motivation**: `mordred-docs/` が flat に肥大化。 docs を読者別に再構成したい。
...
- **移行**: 全ファイルを `git mv` で移送 (履歴保持)、 リポジトリ全体の cross-reference (~50 箇所: src docstring / tests / CI path-trigger / 各 README / `pyproject.toml` Documentation URL) を一括更新、 `mordred-docs/README.md` 索引を user/dev 構成へ書き換え。
