...
In this development checkout the fully-wired venv is `mordred-hermes/.venv`:

```sh
cd <repo-root>            # /Users/.../Mordred-Hermes
mordred-hermes/.venv/bin/hermes-mordred status
```
...
The skill ships in this repo at `mordred-hermes/skills/mordred-status/SKILL.md`.
Install = copy it into the Hermes skills directory:

```sh
mkdir -p ~/.hermes/skills/mordred-status
cp mordred-hermes/skills/mordred-status/SKILL.md ~/.hermes/skills/mordred-status/
```
...
(`hermes-mordred policy dry-run mordred-hermes/skills/mordred-status` → allow,
