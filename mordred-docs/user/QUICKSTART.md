...
- **This repository checked out locally** — the guide runs the copy of Mordred
  you build into `mordred-hermes/.venv`. Don't have the repo yet? Clone it in one
...
The `hermes-mordred` CLI runs from a virtualenv at `mordred-hermes/.venv`, which
...
`uv sync` reads `mordred-hermes/uv.lock` and creates `mordred-hermes/.venv/` with
...
```sh
mordred-hermes/.venv/bin/hermes-mordred --version  # → hermes-mordred 0.1.0a0
```
...
M=mordred-hermes/.venv/bin/hermes-mordred
...
Once you've [built the venv](#build-the-venv), `mordred-hermes/.venv` holds the
...
> `set M mordred-hermes/.venv/bin/hermes-mordred` instead. Then `$M status` works
...
> live in different venvs: Mordred is `mordred-hermes/.venv`, the host agent is
> the **repo-root** `.venv`.

### Build the repo-root venv

Just like `mordred-hermes/.venv` above, the **repo-root** `.venv` that runs the
